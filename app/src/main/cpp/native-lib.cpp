...
Java_com_mgg_ndk_MainActivity_testVariantTest(JNIEnv *env, jobject thiz) {
  ...
  const std::vector<FOREVER::Variant> kTestVector = {int64_t(1L), "one", true, 1.0};
  const std::vector<FOREVER::Variant> kTestComplexVector = {int64_t(2L), "two",
  ...
  g_test_complex_map[FOREVER::Variant::FromMutableBlob(kTestBlobData, kTestBlobSize)] =
          kTestMutableString;
